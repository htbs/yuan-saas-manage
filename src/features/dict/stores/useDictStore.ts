...
import { DictFilterListParams } from "../types";
...
interface DictState {
  ...
  view:
    ...
    | "item_detail";
  editId: string | null;
  queryParams: DictFilterListParams;
  pagination: {
    current: number;
    pageSize: number;
  };
  ...
  setView: (view: DictState["view"], id?: string | null) => void;
  // 更新搜索条件
  setQueryParams: (params: DictFilterListParams) => void;
  // 更新分页
...
export const useDictStore = create<DictState>()(
  persist(
    ...
    {
      name: "dict-management-storage", // 存储在 localStorage 中的 Key
