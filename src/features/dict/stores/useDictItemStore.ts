...
import { DictItemFilterListParams } from "../types";

interface DictItemState {
  // --- view workbench:工作台  ---
  view: "list" | "add" | "edit" | "detail";
  editId: string | null;
  queryParams: DictItemFilterListParams;
  pagination: {
    current: number;
    pageSize: number;
  };
  ...
  setView: (view: DictItemState["view"], id?: string | null) => void;
  // 更新搜索条件
  setQueryParams: (params: DictItemFilterListParams) => void;
  // 更新分页
...
export const useDictItemStore = create<DictItemState>()(
  persist(
    (set) => ({
      // 默认值
      view: "list",
    ...
    {
      name: "dict-item-management-storage", // 存储在 localStorage 中的 Key
